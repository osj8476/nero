...
[중요] 아직 박스 학습 weight가 없는 상태에서도 이 서버는 켜질 수 있게 만들어뒀음.
       기본값은 COCO 사전학습 yolov8n.pt라서 "box" 클래스가 실제로는 안 잡힘(정상 동작).
       → 지금은 서버 기동 / health체크 / perception_node 통신 구조만 검증하는 용도.
       학습 끝나면:
           python vlm_boxyolo.py --port 8002 --model runs/detect/train/weights/best.pt

실행:
    python vlm_boxyolo.py --port 8002
    python vlm_boxyolo.py --port 8002 --model path/to/best.pt --conf 0.4

요구 패키지: ultralytics, fastapi, uvicorn, pydantic, pillow, opencv-python(선택), torch
"""
...
def main():
    ...
    parser.add_argument(
        ...
        help="커스텀 학습 weight 경로 (예: runs/detect/train/weights/best.pt). "
