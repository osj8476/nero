...
- [Grasp Geometry 파이프라인(RANSAC+PCA) 신뢰 불가 판정](grasp_geometry_pipeline.md) — estimate_object_geometry의 major_axis_yaw_deg가 실제 회전과 무관하게 편향 수렴하는 현상 실측 확인(2026-08-31), NoOp segmentation 배경누출 의심, 코드는 유지하되 사용 보류. 2026-09-03 point cloud sanity spike(`tools/PC_SPIKE_RESULT.md`): 실물 D435i depth 검증 → raw 직접입력 불가, `SAM crop → Contact-GraspNet` 1순위, SAM 실기검증 선행
